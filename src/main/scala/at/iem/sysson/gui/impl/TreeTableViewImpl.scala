...
//
//  def apply[S <: Sys[S], T <: TreeLike[S, _, _, T]](tree: T)
//                        (implicit tx: S#Tx, cursor: Cursor[S]): TreeTableView /* [S] */ = {
//
...
//
//      val observer = tree.changed.react { implicit tx => upd =>
...
//    extends ComponentHolder[Component] with TreeTableView /* [S] */ with ModelImpl[Any /* FolderView.Update[S] */] {
//    view =>
//
...
//
//      (elem, v) match {
//        case (f: T#Branch, fv: NodeView.Folder) =>
//          val fe    = f.entity
//          // val path  = parent :+ gv
//          // branchAdded(path, gv)
//          if (!fe.isEmpty) {
