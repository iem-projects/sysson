...
class Variable[A](data: Vec[A], val shape: Vec[Int]) {
  ...
  def read(sections: Vec[Range]): Vec[A] = {
    ...
    Vec.tabulate(sz) { i =>
      val (j, _) = ((0, 1) /: zip) { case ((res, m), (dim, r)) =>
        val add = r((i / m) % dim) * m
        (res + add, m * dim)
      }
      data(j)
...
val v = new Variable[Int](Vec(1 to 24: _*), Vec(2, 3, 4))

assert(v.read(Vec(0 until 0, 0 until 0, 0 until 0)) == Vec())
assert(v.read(Vec(0 to 0, 0 to 0, 0 to 2)) == Vec(1, 2, 3))
assert(v.read(Vec(1 to 1, 1 to 1, 0 to 2)) == Vec(17, 18, 19))
assert(v.read(Vec(1 to 1, 2 to 2, 3 to 3)) == Vec(24))
