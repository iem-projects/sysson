...
minMax.map(_._1).zipWithIndex.map { case (m, i) => m * 1.0 / (i+1).sqrt } .plot()  // yes!
minMax.map(_._2).zipWithIndex.map { case (m, i) => m * 1.0 / (i+1).sqrt } .plot()  // yes!

minMax.map(_._1).zipWithIndex.map { case (m, i) => m * 1.0 / (i+1).sqrt } .min // -2.1
minMax.map(_._2).zipWithIndex.map { case (m, i) => m * 1.0 / (i+1).sqrt } .max // +2.4

grid.map { pi => 1.0 / (pi - 9).sqrt } .map(i => f"$i%1.4f")

Vector(1.0000, 1.0000, 1.0000, 0.7071, 0.7071, 0.5774, 0.3780, 0.3536, 0.3780, 0.3536, 0.3333, 0.3162, 0.3780, 0.3333, 0.3162, 0.3162, 0.5000, 0.4472, 0.4082, 0.3536, 0.5000, 0.5000, 0.4472, 0.3780, 0.4082, 0.3780, 0.3536, 0.3162, 0.5000, 0.4472, 0.4082, 0.3536, 0.3780, 0.3333, 0.3162, 0.3162, 0.5000, 0.4472, 0.4082, 0.3536, 0.5000, 0.3780)
